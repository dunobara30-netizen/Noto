...
const ExerciseHub: React.FC<ExerciseHubProps> = ({ gradeLevel, language, currentTheme }) => {
  ...
  const getButtonClass = () => {
      switch(currentTheme) {
          ...
          case 'steampunk': return 'bg-[#cd7f32] text-[#1a1a1a] border-2 border-[#8b4513] shadow-[inset_0_0_10px_rgba(0,0,0,0.5)]';
          case 'easter': return 'bg-yellow-300 text-green-700 font-bold border-b-4 border-yellow-400 active:border-b-0 active:translate-y-1';
          case 'circus': return 'bg-red-600 border-4 border-yellow-400 text-white font-black hover:bg-red-500';
          case 'newyear': return 'bg-black border border-yellow-500 text-yellow-400 font-bold uppercase tracking-widest hover:bg-slate-900';
            ...
            return (
                <div className="space-y-3 mb-6">
                    {exercise.options?.map((option, idx) => {
                        ...
                        if (currentTheme === 'steampunk') btnClass = "bg-[#2b2b2b] border-2 border-[#cd7f32] text-[#cd7f32] hover:bg-[#3d3d3d] font-mono";
                        if (currentTheme === 'easter') btnClass = "bg-white border-2 border-green-200 text-purple-600 hover:bg-green-50";
                        if (currentTheme === 'circus') btnClass = "bg-white border-4 border-red-500 text-blue-900 hover:bg-yellow-50";
                        if (currentTheme === 'newyear') btnClass = "bg-slate-900 border border-yellow-500/50 text-yellow-100 hover:bg-slate-800";
                        ...
                        if (selectedAnswer === option) {
                            ...
                            else if (currentTheme === 'steampunk') btnClass = "bg-[#cd7f32] text-[#1a1a1a] shadow-[inset_0_0_5px_black]";
                            else if (currentTheme === 'easter') btnClass = "bg-green-100 border-green-400 text-green-700";
                            else if (currentTheme === 'circus') btnClass = "bg-yellow-300 border-yellow-500 text-red-700";
                            else if (currentTheme === 'newyear') btnClass = "bg-yellow-500 text-black font-bold shadow-[0_0_10px_#eab308]";
                        ...
                        return (
                            <button
                                ...
                                disabled={showResult}
                                className={`w-full p-4 sm:p-5 rounded-2xl text-left font-bold transition-all duration-200 flex items-center justify-between group ${btnClass} ${currentTheme === 'plush' || currentTheme === 'music' || currentTheme === 'easter' ? 'rounded-full' : ''}`}
                            >
                ...
                <div className="flex gap-4 mb-6">
                    {['True', 'False'].map((option) => {
                        ...
                        if (isSelected) {
                            ...
                            else if (currentTheme === 'steampunk') btnClass = "bg-[#cd7f32] text-black border-[#8b4513]";
                            else if (currentTheme === 'newyear') btnClass = "bg-yellow-500 text-black";
                        ...
                        return (
                            <button
                                ...
                                disabled={showResult}
                                className={`flex-1 p-6 rounded-2xl font-black text-xl transition-all ${btnClass} ${currentTheme === 'plush' || currentTheme === 'music' || currentTheme === 'easter' ? 'rounded-full' : ''}`}
                            >
                                {localizedOption}
                            </button>
                        );
                        ...
                        <input 
                            ...
                            className={`w-full p-4 sm:p-5 rounded-2xl text-lg font-bold border-2 outline-none transition-all ${
                                showResult 
                                    ...
                                    : (currentTheme === 'plush' ? 'border-dashed border-rose-200 bg-white focus:border-rose-400 focus:ring-4 focus:ring-rose-50 text-stone-700 rounded-full' : (currentTheme === 'music' ? 'border-violet-200 bg-white focus:border-violet-400 focus:ring-4 focus:ring-violet-50 text-violet-700 rounded-full' : (currentTheme === 'steampunk' ? 'bg-[#1a1a1a] border-[#cd7f32] text-[#cd7f32] placeholder-[#8b4513] font-mono' : (currentTheme === 'christmas' ? 'border-red-100 bg-white focus:border-red-400 focus:ring-4 focus:ring-red-50 text-slate-700' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 focus:border-violet-500 focus:ring-4 focus:ring-violet-100 dark:focus:ring-violet-900/20 text-slate-700 dark:text-slate-100'))))
                            }`}
                        />
                        {showResult && (
        ...
        case 'flashcard':
            return (
                <div className="mb-6 perspective-1000">
                    ...
                    >
                        {/* Front */}
                        <div className={`absolute inset-0 backface-hidden bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-3xl shadow-lg flex flex-col items-center justify-center p-6 text-center ${currentTheme === 'plush' ? 'border-dashed border-rose-200' : (currentTheme === 'music' ? 'border-violet-200' : (currentTheme === 'steampunk' ? 'bg-[#2b2b2b] border-[#cd7f32] text-[#cd7f32]' : (currentTheme === 'newyear' ? 'bg-slate-900 border-yellow-500 text-yellow-100' : '')))}`}>
                            ...
                            <span className={`absolute bottom-4 text-xs font-bold flex items-center gap-1 ${currentTheme === 'plush' ? 'text-rose-400' : (currentTheme === 'music' ? 'text-violet-500' : (currentTheme === 'steampunk' ? 'text-[#8b4513]' : (currentTheme === 'newyear' ? 'text-yellow-500' : (currentTheme === 'christmas' ? 'text-red-500' : 'text-violet-500 dark:text-violet-400'))))}`}>
                                <RotateCcw className="w-3 h-3" /> {t.revealCard}
                            </span>
                        </div>

                        {/* Back */}
                        <div className={`absolute inset-0 backface-hidden text-white rounded-3xl shadow-xl flex flex-col items-center justify-center p-6 text-center ${currentTheme === 'plush' ? 'bg-rose-300' : (currentTheme === 'music' ? 'bg-violet-400' : (currentTheme === 'steampunk' ? 'bg-[#cd7f32] text-black' : (currentTheme === 'newyear' ? 'bg-yellow-500 text-black' : (currentTheme === 'christmas' ? 'bg-red-600' : 'bg-violet-600 dark:bg-violet-800'))))}`} style={{ transform: 'rotateY(180deg)' }}>
                            <span className={`text-xs font-bold uppercase tracking-widest mb-4 ${currentTheme === 'plush' || currentTheme === 'music' || currentTheme === 'newyear' ? 'text-black/60' : (currentTheme === 'christmas' ? 'text-red-200' : 'text-violet-200')}`}>Answer</span>
      ...
      <div className="animate-in slide-in-from-bottom-4 duration-500 h-full flex flex-col p-4 sm:p-6 transition-all relative">
        ...
        <div className="mb-6 text-center space-y-2 mt-2 sm:mt-6">
            <h2 className={`text-2xl sm:text-3xl font-black text-transparent bg-clip-text flex items-center justify-center gap-2 ${
                ...
                currentTheme === 'music'
                ? 'text-violet-500' :
                currentTheme === 'steampunk' ? 'text-[#cd7f32]' :
                ...
                <BrainCircuit className={`w-7 h-7 sm:w-8 sm:h-8 ${currentTheme === 'plush' ? 'text-rose-300' : (currentTheme === 'music' ? 'text-violet-400' : (currentTheme === 'steampunk' ? 'text-[#8b4513]' : (currentTheme === 'newyear' ? 'text-yellow-500' : (currentTheme === 'christmas' ? 'text-emerald-500' : 'text-violet-500 dark:text-violet-400'))))}`} />
                {t.studyHub}
            </h2>
          ...
          {subjects.map((sub, idx) => (
            <button
              ...
              className={`p-6 sm:p-8 rounded-[2rem] border-2 shadow-sm hover:shadow-xl transition-all duration-300 group text-left relative overflow-hidden bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:-translate-y-1 active:scale-95 ${
                  ...
                  currentTheme === 'steampunk' ? 'bg-[#2b2b2b] border-[#cd7f32] text-[#cd7f32] hover:bg-[#3d3d3d]' :
                  currentTheme === 'newyear' ? 'bg-slate-900 border-yellow-500/30 hover:border-yellow-500' :
            ...
            >
              ...
              <div className="flex items-start justify-between relative z-10">
                  <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl shadow-sm flex items-center justify-center mb-4 text-white bg-gradient-to-br ${sub.gradient} ${currentTheme === 'plush' || currentTheme === 'music' || currentTheme === 'easter' ? 'rounded-full' : (currentTheme === 'steampunk' ? 'bg-[#cd7f32] text-black rounded-none' : (currentTheme === 'newyear' ? 'bg-black border border-yellow-500 text-yellow-400' : ''))}`}>
                    <sub.icon className="w-7 h-7 sm:w-8 sm:h-8" />
                  </div>
                  <div className={`px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${currentTheme === 'plush' ? 'bg-orange-50 text-stone-500' : (currentTheme === 'music' ? 'bg-violet-50 text-violet-500' : (currentTheme === 'steampunk' ? 'bg-black text-[#cd7f32] border border-[#cd7f32]' : (currentTheme === 'newyear' ? 'bg-yellow-500 text-black' : 'bg-slate-50 dark:bg-slate-700 text-slate-400 dark:text-slate-300')))}`}>
                    {gradeLevel}
                  </div>
              </div>
              
              <h3 className={`text-xl sm:text-2xl font-bold mb-1 relative z-10 ${currentTheme === 'steampunk' ? 'text-[#eaddcf]' : (currentTheme === 'newyear' ? 'text-white' : 'text-slate-800 dark:text-slate-100')}`}>
              ...
              <p className={`text-xs font-bold text-slate-400 dark:text-slate-500 flex items-center gap-1 transition-colors ${
                  ...
                  currentTheme === 'steampunk' ? 'group-hover:text-[#cd7f32]' :
                  currentTheme === 'newyear' ? 'group-hover:text-yellow-400' :
      ...
      return (
        <div className="h-full flex flex-col animate-in zoom-in-95 duration-300 relative p-4 sm:p-6 items-center justify-center text-center transition-all">
            ...
            <div className="relative inline-block mb-6 animate-in zoom-in duration-500">
                <div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-3xl shadow-lg flex items-center justify-center text-white bg-gradient-to-br ${currentTheme === 'plush' || currentTheme === 'music' || currentTheme === 'easter' ? 'rounded-full' : (currentTheme === 'steampunk' ? 'rounded-none border-4 border-[#cd7f32]' : '')} ${subStyle?.gradient || 'from-slate-400 to-slate-500'}`}>
            ...
            <div className="w-full max-w-sm space-y-4">
                ...
                {/* Source Toggle */}
                <div className={`flex p-1 rounded-xl mb-2 border ${currentTheme === 'plush' ? 'bg-orange-50 border-rose-100' : (currentTheme === 'music' ? 'bg-violet-50 border-violet-100' : (currentTheme === 'steampunk' ? 'bg-black border-[#cd7f32]' : (currentTheme === 'newyear' ? 'bg-slate-900 border-yellow-500' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700')))}`}>
                    <button 
                        onClick={() => setSourceMode('topic')}
                        className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
                            sourceMode === 'topic' 
                            ? (currentTheme === 'plush' ? 'bg-white text-rose-400 shadow-sm' : (currentTheme === 'music' ? 'bg-white text-violet-500 shadow-sm' : (currentTheme === 'steampunk' ? 'bg-[#cd7f32] text-black' : (currentTheme === 'newyear' ? 'bg-yellow-500 text-black' : (currentTheme === 'christmas' ? 'bg-white text-red-600 shadow-sm' : 'bg-white dark:bg-slate-600 text-violet-600 dark:text-violet-300 shadow-sm')))))
                            : 'text-slate-400'
                        }`}
                    >
                        {t.topic}
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className={`block w-full pl-12 pr-4 py-3 sm:py-4 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl text-slate-700 dark:text-slate-200 placeholder-slate-400 outline-none transition-all font-bold text-base sm:text-lg shadow-sm ${
                                ...
                                currentTheme === 'steampunk' 
                                ? 'bg-[#1a1a1a] border-[#cd7f32] text-[#cd7f32] font-mono rounded-none focus:bg-black placeholder-[#8b4513]' :
                                currentTheme === 'newyear'
                                ? 'bg-slate-900 border-yellow-500 text-yellow-100 focus:ring-yellow-500 focus:ring-2' :
                                currentTheme === 'christmas' 
                    ...
                    <div className="relative group">
                        <textarea
                            ...
                            className={`block w-full p-4 h-32 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl text-slate-700 dark:text-slate-200 placeholder-slate-400 outline-none transition-all font-medium text-sm shadow-sm resize-none ${
                                ...
                                currentTheme === 'steampunk'
                                ? 'bg-[#1a1a1a] border-[#cd7f32] text-[#cd7f32] font-mono rounded-none' :
                                currentTheme === 'christmas'
                                ? 'focus:border-red-400 focus:ring-4 focus:ring-red-50'
                                : 'focus:border-violet-400 focus:ring-4 focus:ring-violet-50 dark:focus:ring-violet-900/20'
                            }`}
                ...
                {/* Difficulty Selector */}
                <div className={`p-1.5 rounded-2xl flex items-center justify-between gap-1 border ${currentTheme === 'plush' ? 'bg-orange-50 border-rose-100' : (currentTheme === 'music' ? 'bg-violet-50 border-violet-100' : (currentTheme === 'steampunk' ? 'bg-black border-[#cd7f32] rounded-none' : (currentTheme === 'newyear' ? 'bg-slate-900 border-yellow-500' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700')))}`}>
      ...
      {loading ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            <div className="relative mb-8">
                <div className={`absolute inset-0 rounded-full blur-2xl animate-pulse ${currentTheme === 'plush' ? 'bg-rose-200' : (currentTheme === 'music' ? 'bg-violet-200' : (currentTheme === 'steampunk' ? 'bg-[#cd7f32]' : (currentTheme === 'christmas' ? 'bg-red-200' : 'bg-violet-200 dark:bg-violet-900/30')))}`}></div>
                <Loader2 className={`w-16 h-16 sm:w-20 sm:h-20 animate-spin relative z-10 ${currentTheme === 'plush' ? 'text-rose-400' : (currentTheme === 'music' ? 'text-violet-500' : (currentTheme === 'steampunk' ? 'text-[#cd7f32]' : (currentTheme === 'newyear' ? 'text-yellow-500' : (currentTheme === 'christmas' ? 'text-red-600' : 'text-violet-600 dark:text-violet-400'))))}`} />
            </div>
        ...
        <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar px-3 sm:px-6 pt-12 sm:pt-16 pb-4">
            ...
            <div className={`backdrop-blur-sm rounded-[2rem] p-5 sm:p-6 shadow-xl border mb-6 relative animate-in slide-in-from-right-8 ${
                ...
                currentTheme === 'steampunk' ? 'bg-[#1a1a1a] border-[#cd7f32] shadow-[0_0_10px_#cd7f32]' :
                ...
                <div className="flex justify-between items-start mb-4">
                    <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border truncate max-w-[200px] ${
                        ...
                        currentTheme === 'steampunk' ? 'bg-[#2b2b2b] text-[#cd7f32] border-[#8b4513]' :
                        currentTheme === 'newyear' ? 'bg-black text-yellow-400 border-yellow-600' :
                        currentTheme === 'christmas' 
                ...
                {exercise.type !== 'flashcard' && (
                    <h3 className={`text-lg sm:text-2xl font-bold leading-snug mb-2 ${currentTheme === 'plush' ? 'text-stone-700' : (currentTheme === 'music' ? 'text-violet-700' : (currentTheme === 'steampunk' ? 'text-[#cd7f32] font-mono' : (currentTheme === 'newyear' ? 'text-yellow-100' : (currentTheme === 'chalkboard' ? 'text-white' : 'text-slate-800 dark:text-slate-100'))))}`}>
                        {exercise.question}
                    </h3>
                )}
            ...
            {showResult ? (
                <div className="animate-in slide-in-from-bottom-4 duration-500 mt-auto">
                         ...
                         <div className={`p-5 sm:p-6 rounded-3xl border mb-6 ${
                            ...
                            <div className="flex items-center gap-2 mb-2">
                                {isCorrect() 
                                    ...
                                    : <XIcon className="w-5 h-5 text-rose-600 dark:text-rose-400" />
                                }
                                <h4 className={`font-bold ${isCorrect() ? 'text-emerald-700 dark:text-emerald-300' : 'text-rose-700 dark:text-rose-300'}`}>
                    ...
                    <button
                        ...
                        className={`w-full py-4 text-white font-bold rounded-2xl shadow-lg transition-all hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-2 text-lg ${getButtonClass()} ${currentTheme === 'plush' || currentTheme === 'music' || currentTheme === 'easter' ? 'rounded-full' : ''}`}
                    >
                ...
                <div className="mt-auto pt-6">
                    <button
                        onClick={checkAnswer}
                        disabled={!selectedAnswer && !fillInput && !isFlipped}
                        className={`w-full py-4 text-white font-bold rounded-2xl shadow-lg transition-all hover:-translate-y-1 active:scale-95 text-lg disabled:opacity-50 disabled:shadow-none disabled:transform-none ${getButtonClass()} ${currentTheme === 'plush' || currentTheme === 'music' || currentTheme === 'easter' ? 'rounded-full' : ''}`}
                    >
                        {t.checkAnswer}
                    </button>
                </div>
