...
const GradeCalculator: React.FC<GradeCalculatorProps> = ({ onCalculate, isAnalyzing }) => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl shadow-violet-200/50 p-6 border border-white/50 h-[600px] lg:h-full flex flex-col transition-all duration-500 hover:shadow-violet-300/50 animate-in slide-in-from-left-4">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium mt-1">Trage deine Fächer ein</p>
        </div>
      ...
      <div className="mb-8">
        ...
        <div className="relative">
          <select
            ...
            className="w-full p-4 rounded-2xl border-2 border-slate-100 bg-slate-50/50 focus:border-violet-500 focus:ring-4 focus:ring-violet-100 transition-all outline-none text-slate-700 font-semibold appearance-none cursor-pointer"
          >
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-2 mb-6 custom-scrollbar">
        {courses.map((course, idx) => (
          <div 
            key={course.id} 
            className="flex items-center gap-3 p-3 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-violet-200 transition-all duration-300 group animate-in slide-in-from-bottom-2"
          ...
          >
            ...
            <input
              ...
              className="flex-1 bg-transparent border-none focus:ring-0 text-slate-700 font-bold placeholder-slate-300 text-lg"
            />
            <div className="relative">
                <select
                  value={course.grade}
                  onChange={(e) => updateCourse(course.id, 'grade', e.target.value)}
                  className="w-20 bg-slate-50 border-none rounded-xl py-2 px-3 text-lg font-black text-slate-700 focus:ring-2 focus:ring-violet-500 focus:bg-white cursor-pointer text-center appearance-none"
                >
                  {Object.keys(GERMAN_GRADES).map(g => (
                    <option key={g} value={g}>{g}</option>
                  ))}
                </select>
                {/* Custom arrow for small select */}
      ...
      <div className="space-y-4 mt-auto">
        ...
        <button
          ...
          className={`w-full py-5 rounded-2xl font-bold text-white text-lg shadow-lg shadow-violet-300/50 flex items-center justify-center gap-3 transition-all transform hover:-translate-y-1 active:scale-95
            ${isAnalyzing || courses.length === 0 
              ? 'bg-slate-300 cursor-not-allowed shadow-none' 
              : 'bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:shadow-fuchsia-300/50'}`}
        >
          {isAnalyzing ? (
