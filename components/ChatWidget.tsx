...
const formatText = (text: string, isAdmin: boolean) => {
    ...
    // For Admin: We keep the text raw but clean common markdown artifacts
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-['Inter']">
      {isOpen && (
        <div className={`w-[calc(100vw-2rem)] sm:w-96 h-[500px] max-h-[70vh] rounded-2xl shadow-2xl border flex flex-col overflow-hidden mb-4 transition-all animate-in slide-in-from-bottom-10 duration-200 ${isAdmin ? 'border-slate-800' : 'border-slate-200 dark:border-slate-700'}`}>
          ...
          <div className={`${containerTheme.header} p-4 flex justify-between items-center transition-colors`}>
            ...
            <button onClick={() => setIsOpen(false)} className={`hover:bg-white/10 p-1 rounded-lg transition-colors ${containerTheme.headerText}`}>
              <Minimize2 className="w-5 h-5" />
            </button>
          </div>

          {/* Messages */}
          <div className={`flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar ${containerTheme.bg}`}>
            {messages.map((msg) => (
              <MessageItem key={msg.id} msg={msg} isAdmin={isAdmin} />
            ))}
      ...
      <button
        ...
        className={`h-14 w-14 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 hover:scale-105 ${containerTheme.fab} ${isOpen ? 'rotate-90' : ''}`}
      >
