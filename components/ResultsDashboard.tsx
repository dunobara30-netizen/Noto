...
const ResultsDashboard: React.FC<ResultsDashboardProps> = ({ results, gpa, courses, gradeLevel, language }) => {
  ...
  // UK (9-1): Higher is better. Target = GPA + 0.5 (improve)
  // German (1-6): Lower is better. Target = GPA - 0.3 (improve)
  ...
  return (
    <div className="space-y-4 sm:space-y-6 h-auto lg:h-full lg:overflow-y-auto pr-0 sm:pr-3 custom-scrollbar p-4 sm:p-6 pb-24 lg:pb-24 animate-in slide-in-from-right-4 duration-500">
      ...
      {results.archetype && (
        <div className="bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-3xl p-5 sm:p-6 text-white shadow-xl shadow-violet-200 relative overflow-hidden">
      ...
      <div className="bg-white rounded-3xl shadow-lg border border-slate-100 p-5 sm:p-6 flex flex-col md:flex-row gap-6 items-stretch">
         <div className="flex-1 w-full min-h-[180px] sm:h-48 relative">
            ...
            {/* Labels overlay */}
            <div className="absolute bottom-0 left-0 right-0 flex justify-around w-full font-bold text-slate-600 text-sm">
                ...
                <span className="text-emerald-600">{getClosestGradeLabel(targetGradeNum, language)} <span className="text-xs text-emerald-400 font-normal block sm:inline">{t.target}</span></span>
            </div>
         </div>
         <div className="flex-1 space-y-3 sm:space-y-4 w-full flex flex-col justify-center">
             <div className="bg-slate-50 p-3 sm:p-4 rounded-xl border border-slate-100">
                <h4 className="text-xs sm:text-sm font-bold text-violet-600 mb-1">{t.aiAnalysis}</h4>
                <p className="text-slate-600 text-xs sm:text-sm italic leading-relaxed">"{results.advice.summary}"</p>
             </div>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="bg-emerald-50 p-2.5 sm:p-3 rounded-xl text-xs font-bold text-emerald-700 flex items-center gap-2">
                    <CheckCircle className="w-4 h-4 shrink-0" />
                    <span className="truncate">{results.advice.strengths[0] || "Good basis"}</span>
                </div>
                <div className="bg-amber-50 p-2.5 sm:p-3 rounded-xl text-xs font-bold text-amber-700 flex items-center gap-2">
      ...
      <div className="space-y-4 pt-2 sm:pt-4 border-t border-slate-200/60">
        <div className="flex items-center justify-between px-1">
             <h3 className="text-base sm:text-lg font-bold text-slate-700">{t.collegesHeader}</h3>
        </div>
        <div className="grid grid-cols-1 gap-3 sm:gap-4">
          {results.colleges.map((college, index) => (
            <div key={index} className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-100 shadow-sm hover:shadow-md transition-all group">
              <div className="flex justify-between items-start mb-2 gap-2">
                <div className="min-w-0">
                  <h4 className="font-bold text-slate-800 truncate text-sm sm:text-base">{college.name}</h4>
                ...
                <span className={`text-[10px] font-bold px-2 py-1 rounded-full border shrink-0 ${
                    ['Optimistisch', 'Reach'].includes(college.category) ? 'bg-rose-50 text-rose-600 border-rose-100' :
                    ['Realistisch', 'Target'].includes(college.category) ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                    'bg-blue-50 text-blue-600 border-blue-100'
                }`}>
                  {college.category}
                </span>
              </div>
              <p className="text-slate-600 text-xs sm:text-sm mb-2 line-clamp-2 sm:line-clamp-none">{college.reason}</p>
